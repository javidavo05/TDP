...
export class BanescoProvider implements IPaymentProvider {
  method: PaymentMethod = "banesco";

  private apiKey: string;
  private webhookUrl: string;

  constructor() {
    this.apiKey = process.env.BANESCO_API_KEY || "";
    this.webhookUrl = process.env.BANESCO_WEBHOOK_URL || "";
  }
  ...
  async initiatePayment(data: {
    ...
    // TODO: Implement Banesco API integration
    try {
      const response = await fetch("https://api.banesco.com.pa/v1/payments", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`Banesco API error: ${response.statusText}`);
      }
  ...
  async verifyPayment(transactionId: string): Promise<PaymentProviderResponse> {
    try {
      const response = await fetch(`https://api.banesco.com.pa/v1/payments/${transactionId}`, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${this.apiKey}`,
        },
  ...
  async refund(transactionId: string, amount: number): Promise<PaymentProviderResponse> {
    try {
      const response = await fetch(`https://api.banesco.com.pa/v1/payments/${transactionId}/refund`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${this.apiKey}`,
