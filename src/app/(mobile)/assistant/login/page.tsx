...
export default function AssistantLoginPage() {
  ...
  useEffect(() => {
    // Set PWA ID
    PWAAuthService.setPWAId("assistant");
    
    // Check if already logged in
    const savedCreds = PWAAuthService.getCredentials("assistant");
    if (savedCreds && savedCreds.role === "assistant") {
      router.push("/mobile/assistant");
    }
  }, [router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only allow assistants and admins
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-950 dark:to-emerald-900 p-4">
      <div className="w-full max-w-md bg-card rounded-2xl shadow-lg p-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold mb-2">TDP Ayudante</h1>
